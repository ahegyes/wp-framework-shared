...
Pending entries live in [`changelog/`](./changelog) — add via `composer changelog:add:shared` from the monorepo root. Aggregate into a release with `composer changelog:write:shared`.
